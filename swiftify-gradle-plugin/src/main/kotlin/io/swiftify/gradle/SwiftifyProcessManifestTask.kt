 ...
 *
 * This task reads the manifest file produced by SwiftifySymbolProcessor
 * and generates the corresponding Swift code.
 */
abstract class SwiftifyProcessManifestTask : DefaultTask() {
    ...
    fun processManifest() {
        ...
        if (manifest == null || !manifest.exists()) {
            logger.lifecycle("Swiftify: No manifest file found. Run KSP first.")
            return
        }
        ...
        logger.lifecycle("Swiftify: Processing manifest from ${manifest.absolutePath}")

        val declarations = parseManifest(manifest)
    ...
    private fun parseManifest(file: File): List<ParsedDeclaration> {
